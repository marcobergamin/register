...
namespace reg_impl {
template <typename T, int BIT> inline constexpr void make_mask(T &mask) {
  mask |= T{T(1) << BIT};
}
...
template <typename T, std::size_t ADDRESS> class Register final {
public:
  static_assert(ADDRESS % std::alignment_of<T>::value == 0, "Wrong alignment");
  ...
  template <int BIT> static void set_bit() noexcept {
    static_assert(BIT < sizeof(T) * 8U, "Out of bound");
    reg() |= T{T(1) << BIT};
  }

  template <int... BITS> static constexpr T make_mask() {
    T mask{};
    reg_impl::make_mask<T, BITS...>(mask);
    return mask;
  }

  template <int... BITS> static void set_bits() noexcept {
...
template <std::size_t ADDRESS> using RegU64 = Register<std::uint64_t, ADDRESS>;

template <std::size_t ADDRESS> using RegU32 = Register<std::uint32_t, ADDRESS>;

template <std::size_t ADDRESS> using RegU16 = Register<std::uint16_t, ADDRESS>;

template <std::size_t ADDRESS> using RegU8 = Register<std::uint8_t, ADDRESS>;
